...
#Слегка допиленая библиотека OAuth::Lite::ServerUtil
#Изменения касаются функции verify_signature
...
sub verify_signature {
    ...
    syslog('alert',"<request> HTTP METHOD: ".$http_method);
    syslog('alert',"<request> URL: ".$url);

    foreach my $key (keys %{$params}){
	    if ((defined $params->{$key})&&(!ref $params->{$key})) {
		    $params->{$key} = unencode_param($params->{$key});
	    }
    }
=cut	
	foreach my $key (keys %{$params}) {
		if ((defined $params->{$key})&&(!ref $params->{$key})) {
			syslog('alert', "$key = ".$params->{$key});
		} elsif ((defined $params->{$key})&&(ref $params->{$key} eq 'ARRAY')) {
			syslog('alert', "Array found");
			for (my $i=0;$i<@{$params->{$key}};$i++){
				syslog('alert', $params->{$key}->[$i]);
			}
		} elsif ((defined $params->{$key})&&(ref $params->{$key} eq 'HASH')) {
			syslog('alert', "$key {");
			foreach my $key2 (keys %{$params->{$key}}) {
				syslog('alert',"\t$key2 = ".$params->{$key}->{$key2});
			}
			syslog('alert', "}");
		} else {
			syslog('alert', "$key = ".$params->{$key});
...
=cut
	...
	syslog('alert', "<request> CONSUMER_SECRET = ".$consumer_secret);
	my $token_secret     = $args{token_secret} || '';
	syslog('alert', "<request TOKEN_SECRET = ".$token_secret);
#    my $signature_method = $params->{signature_method};
	my $signature_method = "HMAC-SHA1";
	syslog('alert', "<request> HMAC-SHA1 = ".$signature_method);
	my $signature        = unencode_param($params->{signature});
	syslog('alert', "<request> SIGNATURE = ".$signature);
    ...
    my $sign = $method->sign($base_string);

    
    syslog('alert', "Created  SIGN: ".$sign);
    syslog('alert', "Original SIGN: ".$signature);

	syslog ('alert' , "SIGN0 is valid: ".$method->verify($base_string, $sign));
	my $result = $method->verify($base_string, $signature);
	if ($result eq 0 && $base_string =~ /\%3A/){
		$base_string =~ s/\%3A/\:/gi;
		$base_string =~ s/\%2F/\//gi;
		$base_string =~ s/\%26/\&/gi;
		$base_string =~ s/\%3D/\=/gi;
	}
	syslog ('alert' , "BASE_STRING: ".$base_string);
	syslog ('alert' , "SIGN2 is valid: ".$method->verify($base_string, $signature));
