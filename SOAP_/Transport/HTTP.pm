...
# SOAP_::Lite is free software; you can redistribute it
...
package SOAP_::Transport::HTTP::Client;
...
sub new {
    ...
    patch() if $SOAP_::Constants::PATCH_HTTP_KEEPALIVE;
    ...
    die
"SOAP_::Transport::HTTP::Client must inherit from LWP::UserAgent, or one of its subclasses"
      if !$self->isa("LWP::UserAgent");

    $self->agent( join '/', 'SOAP_::Lite', 'Perl',
        $SOAP_::Transport::HTTP::VERSION );
    $self->options( {} );
...
sub send_receive {
  ...
  COMPRESS: {
        ...
        while (1) {
          ...
          # bytelength() function from SOAP_::Utils) and then drop utf8 mark
          # from string (doing pack with 'C0A*' modifier) if length and
          # bytelength are not the same
            my $bytelength = SOAP_::Utils::bytelength($envelope);
            if ($] < 5.008) {
            ...
            #  if !$SOAP_::Constants::DO_NOT_USE_LWP_LENGTH_HACK
            ...
            if ( $method eq 'M-POST' ) {
                ...
                $http_request->header(
                    Man => qq!"$SOAP_::Constants::NS_ENV"; ns=$prefix! );
            ...
            $http_request->header( 'Accept-Encoding' =>
                  [$SOAP_::Transport::HTTP::Client::COMPRESS] )
            ...
            if ( !$http_request->content_type ) {
                $http_request->content_type(
                    join '; ',
                    $SOAP_::Constants::DEFAULT_HTTP_CONTENT_TYPE,
                    !$SOAP_::Constants::DO_NOT_USE_CHARSET && $encoding
                    ? 'charset=' . lc($encoding)
                    : () );
            }
            elsif ( !$SOAP_::Constants::DO_NOT_USE_CHARSET && $encoding ) {
            ...
            SOAP_::Trace::transport($http_request);
            SOAP_::Trace::debug( $http_request->as_string );
            ...
            SOAP_::Trace::transport( $self->http_response );
            SOAP_::Trace::debug( $self->http_response->as_string );
    ...
    my $content =
      ...
      /\b$SOAP_::Transport::HTTP::Client::COMPRESS\b/o
      && $self->options->{is_compress}
...
package SOAP_::Transport::HTTP::Server;
...
sub handle {
    ...
    SOAP_::Trace::debug( $self->request->content );
    ...
    if ( $self->request->method eq 'POST' ) {
        ...
        return $self->response(
            HTTP::Response->new(
                510,    # NOT EXTENDED
"Expected Mandatory header with $SOAP_::Constants::NS_ENV as unique URI"
            ) )
          if $self->request->header('Man') !~
              /^"$SOAP_::Constants::NS_ENV";\s*ns\s*=\s*(\d+)/;
...
# TBD: but what to do with MIME encoded messages in THOSE environments?
    ...
    # TODO - this should query SOAP_::Packager to see what types it supports,
    ...
    SOAP_::Trace::debug($response);

    $self->make_response( $SOAP_::Constants::HTTP_ON_SUCCESS_CODE, $response );
}

sub make_fault {
    my $self = shift;
    $self->make_response(
        $SOAP_::Constants::HTTP_ON_FAULT_CODE => $self->SUPER::make_fault(@_)
    );
    return;
}
...
sub make_response {
    ...
    my $compressed = $self->options->{is_compress}
      ...
      SOAP_::Utils::bytelength $response;
    ...
    $self->response(
        HTTP::Response->new(
            ...
            HTTP::Headers->new(
                ...
                'Content-Type' => join( '; ',
                    'text/xml',
                    !$SOAP_::Constants::DO_NOT_USE_CHARSET
                      && $encoding ? 'charset=' . lc($encoding) : () ),
                'Content-Length' => SOAP_::Utils::bytelength $response
            ),
            ( $] > 5.007 )
...
sub product_tokens {
    join '/', 'SOAP_::Lite', 'Perl', $SOAP_::Transport::HTTP::VERSION;
}

# ======================================================================

package SOAP_::Transport::HTTP::CGI;

use vars qw(@ISA);
@ISA = qw(SOAP_::Transport::HTTP::Server);
...
sub handle {
    ...
    elsif ( defined $SOAP_::Constants::MAX_CONTENT_SIZE
        && $length > $SOAP_::Constants::MAX_CONTENT_SIZE ) {
...
package SOAP_::Transport::HTTP::Daemon;
...
package SOAP_::Transport::HTTP::Apache;
...
# a FastCGI transport class for SOAP_::Lite.
# Updated formatting and removed dead code in new() in 2008
# by Martin Kutter
#
# ======================================================================

package SOAP_::Transport::HTTP::FCGI;

use vars qw(@ISA);
@ISA = qw(SOAP_::Transport::HTTP::CGI);
