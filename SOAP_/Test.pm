...
sub run_for {
  ...
  # ------------------------------------------------------
  my $s = SOAP_::Lite->uri('http://something/somewhere')->proxy($proxy)->on_fault(sub{});
  eval { $s->transport->timeout($SOAP_::Test::TIMEOUT) };
  ...
  eval q!use SOAP_::Lite on_fault => sub{ref $_[1] ? $_[1] : new SOAP_::SOM}; 1! or die;
  ...
  $s->echo(SOAP_::Header->name(somethingelse => 123)
                       ->mustUnderstand(1));
  ...
  eval { A->a };
  ok(!$@ && SOAP_::Lite->self->call->faultstring =~ /Failed to access class \(A\)/);
  ...
  print STDERR "Object autobinding and SOAP_:: prefix test(s)...\n";

  eval "use SOAP_::Lite +autodispatch =>
    uri => 'urn:', proxy => '$proxy'; 1" or die;

  ok(SOAP_::Lite->autodispatched);
  ...
  my $p = My::PingPong->SOAP_::new(10);
  ok(ref $p && $p->SOAP_::next+1 == $p->value);
  ...
  # should return object EXACTLY as after My::PingPong->SOAP_::new(10)
  $p = $s->SOAP_::new(10);
  ok(ref $p && $s->SOAP_::next($p)+1 == $p->value);

  print STDERR "VersionMismatch test(s)...\n";

  {
    local $SOAP_::Constants::NS_ENV = 'http://schemas.xmlsoap.org/new/envelope/';
    my $s = SOAP_::Lite
      -> uri('http://my.own.site.com/My/Examples')
      -> proxy($proxy)
      -> on_fault(sub{})
  ...
  { local $^W; # disable warnings about deprecated AUTOLOADing for nonmethods
    ...
    my @parameters = (
      SOAP_::Data->name(b => 222),
      SOAP_::Data->name(c => 333),
      SOAP_::Data->name(a => 111)
    );
  ...
  if (!eval "pack('U*', 0)") {
    ...
    $s = SOAP_::Lite
     ...
     my $result = $s->echo(SOAP_::Data->type(string => $utf8))->result;
...

  {
    ...
    my $detail = $s->die_with_object()->dataof(SOAP_::SOM::faultdetail . '/[1]');
  ...
  ok($s->call(SOAP_::Data->name('getStateName')->attr({xmlns => 'urn:/My/Examples'}), 1)->result eq 'Alabama');
  ...
  # Funny test.
  # Let's forget about ALL settings we did before with 'use SOAP_::Lite...'
  SOAP_::Lite->self(undef);
  ok(!defined SOAP_::Lite->self);
  ...
  eval { $s->call(SOAP_::Data->name('a:getStateName')) };
...
SOAP_::Test - Test framework for SOAP_::Lite
