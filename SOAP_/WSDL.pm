...
use SOAP_::WSDL::Client;
use SOAP_::WSDL::Expat::WSDLParser;
use Class::Std::Fast constructor => 'none';
use SOAP_::WSDL::XSD::Typelib::Builtin::anySimpleType;
use LWP::UserAgent;
...
{   # just a BLOCK for scoping warnings.
    ...
    # SOAP_::WSDL->new( key => value ) syntax,
    # like SOAP_::Lite does. Class::Std enforces a single hash ref as
    # parameters to new()
    ...
    sub new {
        ...
        my $ident = ident $self;
        $client_of{ $ident } = SOAP_::WSDL::Client->new();
...
sub call {
    ...
    if ( ( ! $outputtree_of{ $ident } )
      ...
      && ( ! $no_dispatch_of{ $ident } ) ) {
        require SOAP_::WSDL::Deserializer::SOM;
        $client->set_deserializer( SOAP_::WSDL::Deserializer::SOM->new() );
    }
    ...
    my (@response) = (blessed $data)
        ...
        : do {
            ...
            $client->call(
                ...
                SOAP_::WSDL::XSD::Typelib::Builtin::anySimpleType->new({
                    value => $content
                }),
                SOAP_::WSDL::XSD::Typelib::Builtin::anySimpleType->new({
                    value => $header
...
SOAP_::WSDL - SOAP with WSDL support
