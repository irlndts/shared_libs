...
sub find_packager {
   ...
   #        Returns:
   #        * the proper SOAP_::Packager instance
}
...
sub package {
    # do nothing
    die "SOAP_::Packager::package() must be implemented";
}
...
package SOAP_::Packager::MIME;

use strict;
use vars qw(@ISA);
@ISA = qw(SOAP_::Packager);
...
sub unpackage {
  ...
  if (lc($entity->head->mime_type) eq 'multipart/form-data') {
    ...
    # warn "I am somewhere in the SOAP_::Packager::MIME code I didn't know I would be in!";
...
sub process_related {
  ...
  # be found, the head/root entity is used as a starting point.
    ...
    # Content-ID (the same approach as in SOAP_::Lite 0.66)
...
sub new {
    ...
    my $self = SOAP_::Packager::new(@_);
    bless $self, $classname;
    $SOAP_::Packager::SUPPORTED_TYPES->{"DIME::Payload"} = 1;
    return $self;
}
...
SOAP_::Packager - this class is an abstract class which allows for multiple types of packaging agents such as MIME and DIME.

=head1 DESCRIPTION

The SOAP_::Packager class is responsible for managing a set of "parts." Parts are
