...
package SOAP_::XMLSchemaApacheSOAP::Deserializer;
...
package SOAP_::XMLSchema::Serializer;
...
package SOAP_::XMLSchema1999::Serializer;
...
sub as_base64 {
    ...
    return [
        $name,
        {
            'xsi:type' => SOAP_::Utils::qualify($self->encprefix => 'base64'),
            %$attr
        },
...
package SOAP_::XMLSchema2001::Serializer;
...
*AUTOLOAD = \&SOAP_::XMLSchema1999::Serializer::AUTOLOAD;
...
sub as_long;        *as_long = \&SOAP_::XMLSchema1999::Serializer::as_long;
sub as_float;       *as_float = \&SOAP_::XMLSchema1999::Serializer::as_float;
sub as_string;      *as_string = \&SOAP_::XMLSchema1999::Serializer::as_string;
sub as_anyURI;      *as_anyURI = \&SOAP_::XMLSchema1999::Serializer::as_anyURI;

# TODO - QNames still don't work for 2001 schema!
sub as_QName;       *as_QName = \&SOAP_::XMLSchema1999::Serializer::as_string;
...
sub disqualify {
    (my $qname = shift) =~ s/^($SOAP_::Constants::NSMASK?)://;
    return $qname;
}
...
# methods for internal tree (SOAP_::Deserializer, SOAP_::SOM and SOAP_::Serializer)
...
package SOAP_::Transport;

use vars qw($AUTOLOAD @ISA);
@ISA = qw(SOAP_::Cloneable);

use Class::Inspector;


sub DESTROY { SOAP_::Trace::objects('()') }
...
use SOAP_::Lite::Deserializer::XMLSchemaSOAP1_2;  
...
sub name {
    ...
    if (@_) {
        ...
        if ($name) {
            ($uri, $name) = SOAP_::Utils::splitlongname($name);
            unless (defined $uri) {
                ($prefix, $name) = SOAP_::Utils::splitqname($name);
...
sub type {
    ...
    if (!defined $self->{_type} && (my @types = grep {/^\{$SOAP_::Constants::NS_XSI_ALL}type$/o} keys %{$self->{_attr}})) {
        $self->{_type} = (SOAP_::Utils::splitlongname(delete $self->{_attr}->{shift(@types)}))[1];
    }
    return $self->{_type};
}
...
BEGIN {
    ...
    for my $method (qw(root mustUnderstand)) {
        ...
        *$method = sub {
        my $attr = $method eq 'root'
            ? "{$SOAP_::Constants::NS_ENC}$method"
            : "{$SOAP_::Constants::NS_ENV}$method";
            my $self = UNIVERSAL::isa($_[0] => __PACKAGE__)
                ? shift->new
                : __PACKAGE__->new;
            if (@_) {
            ...
            $self->{$field} = SOAP_::Lite::Deserializer::XMLSchemaSOAP1_2->as_boolean($self->{_attr}->{$attr})
    ...
    for my $method (qw(actor encodingStyle)) {
        my $field = '_' . $method;
        *$method = sub {
            my $attr = "{$SOAP_::Constants::NS_ENV}$method";
            my $self = UNIVERSAL::isa($_[0] => __PACKAGE__)
                ? shift->new()
                : __PACKAGE__->new();
            if (@_) {
...
package SOAP_::Serializer;
use SOAP_::Lite::Utils;
use Carp ();
use vars qw(@ISA);

@ISA = qw(SOAP_::Cloneable SOAP_::XMLSchema::Serializer);
...
sub BEGIN {
    ...
    # SOAP_::Utils directly.
    # for my $method (qw(qualify overqualify disqualify)) { # import from SOAP_::Utils
    #   *$method = \&{'SOAP_::Utils::'.$method};
    # }
}
...
sub new {
    ...
    $self = bless {
        ...
        _encodingStyle => $SOAP_::Constants::NS_ENC,
        _attr => {
            "{$SOAP_::Constants::NS_ENV}encodingStyle" => $SOAP_::Constants::NS_ENC,
        },
        _namespaces => {},
        _soapversion => SOAP_::Lite->soapversion,
    } => $class;
    $self->register_ns($SOAP_::Constants::NS_ENC,$SOAP_::Constants::PREFIX_ENC);
    $self->register_ns($SOAP_::Constants::NS_ENV,$SOAP_::Constants::PREFIX_ENV)
        if $SOAP_::Constants::PREFIX_ENV;
    $self->xmlschema($SOAP_::Constants::DEFAULT_XML_SCHEMA);
    SOAP_::Trace::objects('()');
...
sub encodingStyle {
    ...
    my $new_style = shift;
    if ($new_style eq "") {
        delete($self->{'_attr'}->{"{$SOAP_::Constants::NS_ENV}encodingStyle"});
    }
    else {
        $self->{'_attr'}->{"{$SOAP_::Constants::NS_ENV}encodingStyle"} = $new_style;
        $self->{'_namespaces'}->{$new_style} = $SOAP_::Constants::PREFIX_ENC;
    }
}
...
sub soapversion {
    ...
    return $self if $self->{_soapversion} eq SOAP_::Lite->soapversion;
    ...
    $self->namespaces({
        $SOAP_::Constants::NS_ENC => $SOAP_::Constants::PREFIX_ENC,
        $SOAP_::Constants::PREFIX_ENV ? ($SOAP_::Constants::NS_ENV => $SOAP_::Constants::PREFIX_ENV) : (),
    });
    $self->xmlschema($SOAP_::Constants::DEFAULT_XML_SCHEMA);
...
sub xmlschema {
    ...
    my @schema;
    if ($_[0]) {
        @schema = grep {/XMLSchema/ && /$_[0]/} keys %SOAP_::Constants::XML_SCHEMAS;
    ...
    my $class = exists $SOAP_::Constants::XML_SCHEMAS{$self->{_xmlschema}}
        ? $SOAP_::Constants::XML_SCHEMAS{$self->{_xmlschema}} . '::Serializer'
        : $self;
...
sub envprefix {
    my $self = shift->new();
    return $self->namespaces->{$SOAP_::Constants::NS_ENV} unless @_;
    $self->namespaces->{$SOAP_::Constants::NS_ENV} = shift;
    return $self;
}

sub encprefix {
    my $self = shift->new();
    return $self->namespaces->{$SOAP_::Constants::NS_ENC} unless @_;
    $self->namespaces->{$SOAP_::Constants::NS_ENC} = shift;
    return $self;
}
...
sub maptypetouri {
    ...
    my($prefix, $name) = SOAP_::Utils::splitqname($type);

    unless (defined $prefix) {
        $name =~ s/__|\./::/g;
        $self->maptype->{$name} = $simple
            ? die "Schema/namespace for type '$type' is not specified\n"
            : $SOAP_::Constants::NS_SL_PERLTYPE
                unless exists $self->maptype->{$name};
        $type = $self->maptype->{$name}
            ? SOAP_::Utils::qualify($self->namespaces->{$self->maptype->{$name}} ||= gen_ns, $type)
            : undef;
    }
    return $type;
}
...
sub encode_object {
    ...
    if (UNIVERSAL::isa($object => 'SOAP_::Data')) {
        # use $object->SOAP_::Data:: to enable overriding name() and others in inherited classes
        $object->SOAP_::Data::name($name)
            unless defined $object->SOAP_::Data::name;

        # apply ->uri() and ->prefix() which can modify name and attributes of
        # element, but do not modify SOAP_::Data itself
        my($name, $attr) = $self->fixattrs($object);
        $attr = $self->attrstoqname($attr);

        my @realvalues = $object->SOAP_::Data::value;
        return [$name || gen_name, $attr] unless @realvalues;

        my $method = "as_" . ($object->SOAP_::Data::type || '-'); # dummy type if not defined
        # try to call method specified for this type
        no strict qw(refs);
        my @values = map {
            # store null/nil attribute if value is undef
            local $attr->{SOAP_::Utils::qualify(xsi => $self->xmlschemaclass->nilValue)} = $self->xmlschemaclass->as_undef(1)
                unless defined;
            $self->can($method) && $self->$method($_, $name || gen_name, $object->SOAP_::Data::type, $attr)
                || $self->typecast($_, $name || gen_name, $object->SOAP_::Data::type, $attr)
                || $self->encode_object($_, $name, $object->SOAP_::Data::type, $attr)
        } @realvalues;
        $object->SOAP_::Data::signature([map {join $;, $_->[0], SOAP_::Utils::disqualify($_->[1]->{'xsi:type'} || '')} @values]) if @values;
    ...
    if (UNIVERSAL::isa($object => 'REF') || UNIVERSAL::isa($object => 'SCALAR')) {
        ...
        # Added in SOAP_::Lite 0.65_6 to fix an XMLRPC bug
...
sub encode_scalar {
    ...
    # null reference
    return [$name, {%$attr, SOAP_::Utils::qualify(xsi => $schemaclass->nilValue) => $schemaclass->as_undef(1)}] unless defined $value;
...
sub encode_array {
    ...
    $arraytype = sprintf "%s\[$num]", keys %types > 1 || $arraytype eq '-' ? SOAP_::Utils::qualify(xsd => $self->xmlschemaclass->anyTypeValue) : $arraytype;

    # $type = SOAP_::Utils::qualify($self->encprefix => 'Array') if $self->autotype && !defined $type;
    $type = qualify($self->encprefix => 'Array') if !defined $type;
    return [$name || SOAP_::Utils::qualify($self->encprefix => 'Array'),
          {
              SOAP_::Utils::qualify($self->encprefix => 'arrayType') => $arraytype,
...
sub encode_literal_array {
    ...
    if ($self->autotype) {
        ...
        $arraytype = sprintf "%s\[$num]", keys %types > 1 || $arraytype eq '-'
            ? SOAP_::Utils::qualify(xsd => $self->xmlschemaclass->anyTypeValue)
            : $arraytype;

        $type = SOAP_::Utils::qualify($self->encprefix => 'Array')
            if !defined $type;
...
sub encode_hash {
    ...
    if ($self->autotype && grep {!/$SOAP_::Constants::ELMASK/o} keys %$hash) {
...
sub as_map {
    ...
    my $prefix = ($self->namespaces->{$SOAP_::Constants::NS_APS} ||= 'apachens');
    my @items = map {
        $self->encode_object(
            SOAP_::Data->type(
                ordered_hash => [
...
sub fixattrs {
    my $self = shift;
    my $data = shift;
    my($name, $attr) = ($data->SOAP_::Data::name, {%{$data->SOAP_::Data::attr}});
...
sub toqname {
    ...
    return SOAP_::Utils::qualify $self->namespaces->{$1} ||= gen_ns, $2;
}
...
sub tag {
    ...
    die "Element '$tag' can't be allowed in valid XML message. Died."
        if $tag !~ /^(?![xX][mM][lL])$SOAP_::Constants::NSMASK$/o;
    ...
    if ($level == 1) {
        ...
        foreach (keys %$namespaces) {
            $attrs->{SOAP_::Utils::qualify(xmlns => $namespaces->{$_})} = $_
        }
    ...
    my $tagattrs = join($tagjoiner, '',
        map { sprintf '%s="%s"', $_, SOAP_::Utils::encode_attribute($attrs->{$_}) }
...
sub uriformethod {
    ...
    my $method_is_data = ref $_[0] && UNIVERSAL::isa($_[0] => 'SOAP_::Data');

    # drop prefix from method that could be string or SOAP_::Data object
    my($prefix, $method) = $method_is_data
        ? ($_[0]->prefix, $_[0]->name)
        : SOAP_::Utils::splitqname($_[0]);

    my $attr = {reverse %{$self->namespaces}};
    # try to define namespace that could be stored as
    #   a) method is SOAP_::Data
...
sub serialize { SOAP_::Trace::trace('()');
    my $self = shift->new;
...
sub envelope {
    ...
    for (@_) {
        ...
        if (defined($_) && ref($_) && UNIVERSAL::isa($_ => 'SOAP_::Header')) {
            push(@header, $_);
        }
        ...
        # Find all the SOAP Body elements
            ...
            push(@parameters, $_);
            # push (@parameters, SOAP_::Utils::encode_data($_));
        }
    }
    my $header = @header ? SOAP_::Data->set_value(@header) : undef;
    my($body,$parameters);
    if ($type eq 'method' || $type eq 'response') {
        SOAP_::Trace::method(@parameters);

        my $method = shift(@parameters);
        #  or die "Unspecified method for SOAP call\n";

        $parameters = @parameters ? SOAP_::Data->set_value(@parameters) : undef;
        if (!defined($method)) {}
        elsif (UNIVERSAL::isa($method => 'SOAP_::Data')) {
            $body = $method;
        }
        elsif ($self->use_default_ns) {
            if ($self->{'_ns_uri'}) {
                $body = SOAP_::Data->name($method)
            ...
            #      $body = SOAP_::Data->name($method)->uri($self->{'_ns_uri'});
        ...
        # proposed resolution for [ 1700326 ] encode_data called incorrectly in envelope
        #    $body->set_value(SOAP_::Utils::encode_data($parameters ? \$parameters : ()))
        #      if $body;
        # must call encode_data on nothing to enforce xsi:nil="true" to be set.
        $body->set_value($parameters ? \$parameters : SOAP_::Utils::encode_data()) if $body;
    }
    elsif ($type eq 'fault') {
        SOAP_::Trace::fault(@parameters);
        # -> attr({'xmlns' => ''})
        # Parameter order fixed thanks to Tom Fischer
        $body = SOAP_::Data-> name(SOAP_::Utils::qualify($self->envprefix => 'Fault'))
          -> value(\SOAP_::Data->set_value(
                SOAP_::Data->name(faultcode => SOAP_::Utils::qualify($self->envprefix => $parameters[0]))->type(""),
                SOAP_::Data->name(faultstring => SOAP_::Utils::encode_data($parameters[1]))->type(""),
                defined($parameters[3])
                    ? SOAP_::Data->name(faultactor => $parameters[3])->type("")
                    : (),
                defined($parameters[2])
                    ? SOAP_::Data->name(detail => do{
                        my $detail = $parameters[2];
                        ref $detail
                            ? \$detail
                            : SOAP_::Utils::encode_data($detail)
                    })
                    : (),
        ));
    }
    elsif ($type eq 'freeform') {
        SOAP_::Trace::freeform(@parameters);
        $body = SOAP_::Data->set_value(@parameters);
    }
    elsif (!defined($type)) {
    ...
    $self->seen({}); # reinitialize multiref table
    # Build the envelope
    # Right now it is possible for $body to be a SOAP_::Data element that has not
    # XML escaped any values. How do you remedy this?
    my($encoded) = $self->encode_object(
        SOAP_::Data->name(
            SOAP_::Utils::qualify($self->envprefix => 'Envelope') => \SOAP_::Data->value(
                ($header
                    ? SOAP_::Data->name( SOAP_::Utils::qualify($self->envprefix => 'Header') => \$header)
                    : ()
                ),
                ($body
                    ? SOAP_::Data->name(SOAP_::Utils::qualify($self->envprefix => 'Body') => \$body)
                    : SOAP_::Data->name(SOAP_::Utils::qualify($self->envprefix => 'Body')) ),
            )
        )->attr($self->attr)
    );
    ...
    # Sometimes SOAP_::Serializer is invoked statically when there is no context.
...
sub xmlparser {
    my $self = shift;
    return eval {
        $SOAP_::Constants::DO_NOT_USE_XML_PARSER
            ? undef
            : do {
                require XML::Parser;
                XML::Parser->new() }
            ...
            || die "XML::Parser is not @{[$SOAP_::Constants::DO_NOT_USE_XML_PARSER ? 'used' : 'available']} and ", $@;
}
...
sub final {
    ...
    # clean handlers, otherwise SOAP_::Parser won't be deleted:
    # it refers to XML::Parser which refers to subs from SOAP_::Parser
...
sub BEGIN {
    ...
    for my $method (qw(o_child o_value o_lname o_lattr o_qname)) { # import from SOAP_::Utils
        *$method = \&{'SOAP_::Utils::'.$method};
    }
...
sub headerof { # SOAP_::Header is the same as SOAP_::Data, so just rebless it
    wantarray
        ? map { bless $_ => 'SOAP_::Header' } shift->dataof(@_)
        : do { # header returned by ->dataof can be undef in scalar context
            my $header = shift->dataof(@_);
            ref $header ? bless($header => 'SOAP_::Header') : undef;
        };
}
...
sub namespaceuriof {
    ...
    $self->match(shift) if @_;
    return wantarray
        ? map {(SOAP_::Utils::splitlongname(o_lname($_)))[0]} @{$self->{_current}}
        : @{$self->{_current}} ? (SOAP_::Utils::splitlongname(o_lname($self->{_current}->[0])))[0] : undef;
}
...
sub _as_data {
    ...
    if ( defined o_child($node) ) {
        ...
        $data->set_value( \SOAP_::Data->value(@children) );
    }
    else {
...
sub _traverse_tree {
    ...
    grep {defined}
        ...
        grep {!ref o_lattr($_) ||
            !exists o_lattr($_)->{"{$SOAP_::Constants::NS_ENC}root"} ||
            o_lattr($_)->{"{$SOAP_::Constants::NS_ENC}root"} ne '0'}
        @$pointer;
}

# ======================================================================

package SOAP_::Deserializer;
...
sub new {
    ...
    return bless {
        '_ids'        => {},
        '_hrefs'      => {},
        '_parser'     => SOAP_::Parser->new,
        '_xmlschemas' => {
            $SOAP_::Constants::NS_APS => 'SOAP_::XMLSchemaApacheSOAP::Deserializer',
#            map {
#                $_ => $SOAP_::Constants::XML_SCHEMAS{$_} . '::Deserializer'
#              } keys %SOAP_::Constants::XML_SCHEMAS
            map {
                $_ => 'SOAP_::Lite::Deserializer::' . $SOAP_::Constants::XML_SCHEMA_OF{$_}
              } keys %SOAP_::Constants::XML_SCHEMA_OF
...
# Returns the envelope and populates SOAP_::Packager with parts
sub decode_parts {
    my $self = shift;
    ...
    foreach (@{$self->context->packager->parts}) {
        ...
        my $part = lc($type) eq 'text/xml' && !$SOAP_::Constants::DO_NOT_PROCESS_XML_IN_MIME
...
# deserialize returns a SOAP_::SOM object and parses straight
# text as input
sub deserialize {
    SOAP_::Trace::trace('()');
    my $self = shift->new;
...
sub decode_object {
    ...
    foreach (keys %$attrs) {
        next unless m/^($SOAP_::Constants::NSMASK?):($SOAP_::Constants::NSMASK)$/;

    $1 =~ /^[xX][mM][lL]/ ||
        $uris{$1} &&
            do {
                $attrs->{SOAP_::Utils::longname($uris{$1}, $2)} = do {
                    my $value = $attrs->{$_};
                    $2 ne 'type' && $2 ne 'arrayType'
                        ? $value
                        : SOAP_::Utils::longname($value =~ m/^($SOAP_::Constants::NSMASK?):(${SOAP_::Constants::NSMASK}(?:\[[\d,]*\])*)/
    ...
    my $ns = ($name =~ s/^($SOAP_::Constants::NSMASK?):// ? $1 : '');
    $ref->[ _NAME ] = SOAP_::Utils::longname(
        $ns
...
sub decode_value {
    ...
    if (++$level == 1) {
        my($namespace, $envelope) = SOAP_::Utils::splitlongname($name);
        SOAP_::Lite->soapversion($namespace) if $envelope eq 'Envelope' && $namespace;
    }

    # check encodingStyle
    # future versions may bind deserializer to encodingStyle
    my $encodingStyle = $attrs->{"{$SOAP_::Constants::NS_ENV}encodingStyle"} || "";
    my (%union,%isect);
    ...
    foreach my $e (@SOAP_::Constants::SUPPORTED_ENCODING_STYLES, split(/ +/,$encodingStyle)) {
        $union{$e}++ && $isect{$e}++;
    }
    die "Unrecognized/unsupported value of encodingStyle attribute '$encodingStyle'"
        if defined($encodingStyle) && length($encodingStyle) > 0 && !%isect &&
            !(SOAP_::Lite->soapversion == 1.1 && $encodingStyle =~ /(?:^|\b)$SOAP_::Constants::NS_ENC/);

    # removed to provide literal support in 0.65
    #$encodingStyle !~ /(?:^|\b)$SOAP_::Constants::NS_ENC/;
    ...
    my ($type) = grep { defined }
        map($attrs->{$_}, sort grep {/^\{$SOAP_::Constants::NS_XSI_ALL\}type$/o} keys %$attrs),
           $name =~ /^\{$SOAP_::Constants::NS_ENC\}/ ? $name : $arraytype;
    ...
    my ($schema, $class) = SOAP_::Utils::splitlongname($type) if $type;
    ...
    return undef if grep {
        /^$SOAP_::Constants::NS_XSI_NILS$/ && do {
    ...
    if (exists $attrs->{"{$SOAP_::Constants::NS_ENC}arrayType"}) {
        my $res = [];
        ...
        my($type, $multisize) = $attrs->{"{$SOAP_::Constants::NS_ENC}arrayType"}
            =~ /^(.+)\[(\d*(?:,\d+)*)\](?:\[(?:\d+(?:,\d+)*)\])*$/
                or die qq!Unrecognized/unsupported format of arrayType attribute '@{[$attrs->{"{$SOAP_::Constants::NS_ENC}arrayType"}]}'\n!;
        ...
        # sparse (position)
        if (ref $children && exists SOAP_::Utils::o_lattr($children->[0])->{"{$SOAP_::Constants::NS_ENC}position"}) {
            my @new;
            for (my $pos = 0; $pos < @$children; $pos++) {
                # TBD implement position in multidimensional array
                my($position) = SOAP_::Utils::o_lattr($children->[$pos])->{"{$SOAP_::Constants::NS_ENC}position"} =~ /^\[(\d+)\]$/
        ...
        my($offset) = $attrs->{"{$SOAP_::Constants::NS_ENC}offset"} =~ /^\[(\d+)\]$/
            if exists $attrs->{"{$SOAP_::Constants::NS_ENC}offset"};
    ...
    elsif ($name =~ /^\{$SOAP_::Constants::NS_ENC\}Struct$/
...
package SOAP_::Client;


use SOAP_::Lite::Utils;

$VERSION = $SOAP_::Lite::VERSION;
sub BEGIN {
...
package SOAP_::Server::Object;

sub gen_id; *gen_id = \&SOAP_::Serializer::gen_id;
...
sub objects_by_reference {
    ...
    while (@_) {
        @alive{shift()} = ref $_[0]
            ? shift
            : sub {
                $_[1]-$_[$_[5] ? 5 : 4] > $SOAP_::Constants::OBJS_BY_REF_KEEPALIVE
            }
    }
    keys %alive;
}
...
package SOAP_::Server::Parameters;

sub byNameOrOrder {
    unless (UNIVERSAL::isa($_[-1] => 'SOAP_::SOM')) {
...
sub initialize {
    return (
        packager => SOAP_::Packager::MIME->new,
        transport => SOAP_::Transport->new,
        serializer => SOAP_::Serializer->new,
        deserializer => SOAP_::Deserializer->new,
        on_action => sub { ; },
        on_dispatch => sub {
            return;
        },
...
sub objects_by_reference {
    ...
    $self = $self->new() if not ref $self;
    @_
        ? (SOAP_::Server::Object->objects_by_reference(@_), return $self)
        : SOAP_::Server::Object->objects_by_reference;
}
...
sub handle {
    ...
    local $SOAP_::Constants::DEFAULT_XML_SCHEMA
        = $SOAP_::Constants::DEFAULT_XML_SCHEMA;
    ...
    my $result = eval {
        ...
        die SOAP_::Fault
            ->faultcode($SOAP_::Constants::FAULT_VERSION_MISMATCH)
            ->faultstring($@)
                if $@ && $@ =~ /^$SOAP_::Constants::WRONG_VERSION/;
        ...
        $self->serializer->soapversion(SOAP_::Lite->soapversion);
        $self->serializer->xmlschema($SOAP_::Constants::DEFAULT_XML_SCHEMA
            = $self->deserializer->xmlschema)
                if $self->deserializer->xmlschema;

        die SOAP_::Fault
            ->faultcode($SOAP_::Constants::FAULT_MUST_UNDERSTAND)
            ->faultstring("Unrecognized header has mustUnderstand attribute set to 'true'")
            if !$SOAP_::Constants::DO_NOT_CHECK_MUSTUNDERSTAND &&
                grep {
                    $_->mustUnderstand
                    && (!$_->actor || $_->actor eq $SOAP_::Constants::NEXT_ACTOR)
                } $request->dataof($som->headers);

        die "Can't find method element in the message"
            unless $request->match($som->method);
        # TODO - SOAP_::Dispatcher plugs in here
        ...
        my @results = eval {
            ...
            # SOAP_::Trace::dispatch($fullname);
            SOAP_::Trace::parameters(@parameters);

            push @parameters, $request
                if UNIVERSAL::isa($class => 'SOAP_::Server::Parameters');

            no strict qw(refs);
            SOAP_::Server::Object->references(
                defined $parameters[0]
                && ref $parameters[0]
                && UNIVERSAL::isa($parameters[0] => $class)
                    ? do {
                        my $object = shift @parameters;
                        SOAP_::Server::Object->object(ref $class
                            ? $class
                            : $object
                        )->$method_name(SOAP_::Server::Object->objects(@parameters)),

                        # send object back as a header
                        # preserve name, specify URI
                        SOAP_::Header
                            ->uri($SOAP_::Constants::NS_SL_HEADER => $object)
                            ->name($request->dataof($som->method.'/[1]')->name)
                    } # end do block

                    # SOAP_::Dispatcher will plug-in here as well
                    # $handler->dispatch(SOAP_::Server::Object->objects(@parameters)
                    : $class->$method_name(SOAP_::Server::Object->objects(@parameters)) );
        }; # end eval block
        SOAP_::Trace::result(@results);
        ...
        die ref $@
            ...
            : $@ =~ /^Can\'t locate object method "$method_name"/
                ...
                : SOAP_::Fault->faultcode($SOAP_::Constants::FAULT_SERVER)->faultstring($@)
                    if $@;
    ...
    return $self->make_fault($SOAP_::Constants::FAULT_CLIENT, $@) unless ref $@;

    # died with SOAP_::Fault
    return $self->make_fault($@->faultcode   || $SOAP_::Constants::FAULT_SERVER,
        $@->faultstring || 'Application error',
        $@->faultdetail, $@->faultactor)
    if UNIVERSAL::isa($@ => 'SOAP_::Fault');

    # died with complex detail
    return $self->make_fault($SOAP_::Constants::FAULT_SERVER, 'Application error' => $@);
...
package SOAP_::Custom::XML::Deserializer;

use vars qw(@ISA);
@ISA = qw(SOAP_::Deserializer);
...
sub decode_value {
    ...
    SOAP_::Custom::XML::Data
        -> SOAP_::Data::name($name)
        -> attr($attrs)
...
package SOAP_::Schema::Deserializer;

use vars qw(@ISA);
@ISA = qw(SOAP_::Custom::XML::Deserializer);

# ======================================================================

package SOAP_::Schema::WSDL;
...
sub import {
    ...
    foreach (@a) {
        ...
        $root->SOAP_::Data::name eq 'definitions' ? $s->set_value($s->value, $root->value) :
        $root->SOAP_::Data::name eq 'schema' ? do { # add <types> element if there is no one
        $s->set_value($s->value, $self->deserializer->deserialize('<types></types>')->root) unless $s->types;
        $s->types->set_value($s->types->value, $root) } :
        die "Don't know what to do with '@{[$root->SOAP_::Data::name]}' in schema imported from '$location'\n";
    }
...
FAKE
    ...
    foreach (@services) {
        ...
        foreach ($_->port) {
            ...
            my $binding = SOAP_::Utils::disqualify($_->binding);
            ...
            foreach ($s->binding) {
                ...
                my $porttype = SOAP_::Utils::disqualify($_->type);
                foreach ($_->operation) {
                    my $opername = $_->name;
                    ...
                    foreach ($s->portType) {
                        ...
                        foreach ($_->operation) {
                            ...
                            my $inputmessage = SOAP_::Utils::disqualify($_->input->message);
                            foreach my $msg ($s->message) {
                                next unless $msg->name eq $inputmessage;
                                if ($invocationStyle eq "document" && $encodingStyle eq "literal") {
#                  warn "document/literal support is EXPERIMENTAL in SOAP_::Lite"
#                  if !$has_warned && ($has_warned = 1);
                                    my ($input_ns,$input_name) = SOAP_::Utils::splitqname($msg->part->element);
                                    ...
                                    #warn "$invocationStyle/$encodingStyle is not supported in this version of SOAP_::Lite";
                                    @parts = $msg->part;
                                    $services{$opername}->{parameters} = [ @parts ];
                                }
                            }
...
# Naming? SOAP_::Service::Schema?
package SOAP_::Schema;
...
sub new {
    ...
    unless (ref $self) {
        ...
        $self = bless {
            '_deserializer' => SOAP_::Schema::Deserializer->new,
...
sub schema {
    warn "SOAP_::Schema->schema has been deprecated. "
        . "Please use SOAP_::Schema->schema_url instead.";
...
sub parse {
    ...
    $self->services({SOAP_::Schema::WSDL->base($self->schema_url)->parse($s, @_)});
}
...
sub load {
    ...
    foreach (keys %{$self->services || Carp::croak 'Nothing to load. Schema is not specified'}) {
        ...
        if ($self->cache_dir) {
            ...
            my $ttl = $self->cache_ttl || $SOAP_::Constants::DEFAULT_CACHE_TTL;
...
sub generate_stub {
    ...
    $self->{'_stub'} = <<"EOP";
package $package;
# Generated by SOAP_::Lite (v$SOAP_::Lite::VERSION) for Perl -- soaplite.com
...
EOP
    ...
    foreach my $service (keys %$services) {
        ...
        foreach (@{$services->{$service}{parameters}}) {
            ...
            next unless ref $_;
            $self->{'_stub'} .= "      SOAP_::Data->new(name => '".$_->name."', type => '".$_->type."', attr => {";
            $self->{'_stub'} .= do {
                my %attr = %{$_->attr};
                join(', ', map {"'$_' => '$attr{$_}'"}
                    grep {/^xmlns:(?!-)/}
...
@ISA = qw(Exporter SOAP_::Lite);
@EXPORT_OK = (keys %methods);
%EXPORT_TAGS = ('all' => [@EXPORT_OK]);

sub _call {
    my ($self, $method) = (shift, shift);
    my $name = UNIVERSAL::isa($method => 'SOAP_::Data') ? $method->name : $method;
    ...
    foreach my $param (@_) {
        if (@templates) {
            ...
            my ($prefix,$typename) = SOAP_::Utils::splitqname($template->type);
...
EOP
    ...
    foreach my $key (keys %{$namespaces}) {
        my ($ns,$prefix) = SOAP_::Utils::splitqname($key);
    ...
    UNIVERSAL::isa($som => 'SOAP_::SOM') ? wantarray ? $som->paramsall : $som->result : $som;
}
...
my $soap; # shared between SOAP and SOAP_::Lite packages

{
    ...
    *AUTOLOAD = sub {
        ...
        my $soap = ref $_[0] && UNIVERSAL::isa($_[0] => 'SOAP_::Lite')
            ? $_[0]
            : $soap
                || die "SOAP_:: prefix shall only be used in combination with +autodispatch option\n";

        my $uri = URI->new($soap->uri);
        my $currenturi = $uri->path;
        $package = ref $_[0] && UNIVERSAL::isa($_[0] => 'SOAP_::Lite')
            ? $currenturi
            : $package eq 'SOAP'
                ? ref $_[0] || ($_[0] eq 'SOAP'
        ...
        # drop first parameter if it's a class name
        {
            ...
            shift @_ if @_ && !ref $_[0] && ($_[0] eq $pack || $_[0] eq 'SOAP')
                || ref $_[0] && UNIVERSAL::isa($_[0] => 'SOAP_::Lite');
        }
...
sub autodispatched { \&{*UNIVERSAL::AUTOLOAD} eq \&{*SOAP_::AUTOLOAD} };

sub soapversion {
    my $self = shift;
    my $version = shift or return $SOAP_::Constants::SOAP_VERSION;

    ($version) = grep {
        $SOAP_::Constants::SOAP_VERSIONS{$_}->{NS_ENV} eq $version
        } keys %SOAP_::Constants::SOAP_VERSIONS
            unless exists $SOAP_::Constants::SOAP_VERSIONS{$version};

    die qq!$SOAP_::Constants::WRONG_VERSION Supported versions:\n@{[
        join "\n", map {"  $_ ($SOAP_::Constants::SOAP_VERSIONS{$_}->{NS_ENV})"} keys %SOAP_::Constants::SOAP_VERSIONS
        ]}\n!
        unless defined($version) && defined(my $def = $SOAP_::Constants::SOAP_VERSIONS{$version});

    foreach (keys %$def) {
        eval "\$SOAP_::Constants::$_ = '$SOAP_::Constants::SOAP_VERSIONS{$version}->{$_}'";
    }

    $SOAP_::Constants::SOAP_VERSION = $version;

    return $self;
}

BEGIN { SOAP_::Lite->soapversion(1.1) }

sub import {
    my $pkg = shift;
    my $caller = caller;
    no strict 'refs';
    # emulate 'use SOAP_::Lite 0.99' behavior
    ...
    while (@_) {
        ...
        if ($command eq 'autodispatch' || $command eq 'dispatch_from') {
            ...
            foreach (keys %{SOAP_::Schema->schema_url(shift(@parameters))->parse(@parameters)->load->services}) {
                $_->export_to_level(1, undef, ':all');
            }
        }
        elsif ($command eq 'debug' || $command eq 'trace') {
            SOAP_::Trace->import(@parameters ? @parameters : 'all');
        }
...
sub new {
    ...
    unless (ref $self) {
        ...
        $self = ref($soap) eq $class ? $soap->clone : {
            _transport    => SOAP_::Transport->new,
            _serializer   => SOAP_::Serializer->new,
            _deserializer => SOAP_::Deserializer->new,
            _packager     => SOAP_::Packager::MIME->new,
            _schema       => undef,
            _autoresult   => 0,
...
sub AUTOLOAD {
    ...
    *$AUTOLOAD = sub {
        ...
        return $self->autoresult && UNIVERSAL::isa($som => 'SOAP_::SOM')
...
sub call {
    ...
    die "A service address has not been specified either by using SOAP_::Lite->proxy() or a service description)\n"
        unless defined $self->proxy && UNIVERSAL::isa($self->proxy => 'SOAP_::Client');
    ...
    if (!$self->transport->is_success || # transport fault
        ...
        UNIVERSAL::isa($result => 'SOAP_::SOM') && $result->fault) {
    ...
    if (UNIVERSAL::isa($result => 'SOAP_::SOM')
        && ($result->paramsout || $result->headers)
        && $serializer->signature) {
        my $num = 0;
        my %signatures = map {$_ => $num++} @{$serializer->signature};
        for ($result->dataof(SOAP_::SOM::paramsout), $result->dataof(SOAP_::SOM::headers)) {
            ...
            if (exists $signatures{$signature}) {
                ...
                # fillup parameters
                UNIVERSAL::isa($_[$param] => 'SOAP_::Data')
                    ? $_[$param]->SOAP_::Data::value($value)
...
sub required {
  foreach (qw(
    ...
    HTTP::Daemon Compress::Zlib SOAP_::Transport::HTTP
...
sub new    { required; SOAP_::Lite->new(@_) }
...
sub data   { SOAP_::Data->new(@_) }

sub header { SOAP_::Header->new(@_) }
...
SOAP_::Lite - Perl's Web Services Toolkit

=head1 DESCRIPTION

SOAP_::Lite is a collection of Perl modules which provides a simple and
lightweight interface to the Simple Object Access Protocol (SOAP) both on
client and server side.

=head1 PERL VERSION WARNING

SOAP_::Lite 0.71 will be the last version of SOAP_::Lite running on perl 5.005

Future versions of SOAP_::Lite will require at least perl 5.6.0
...
L<SOAP_::Lite> - Main class provides all logic

L<SOAP_::Transport> - Transport backend

L<SOAP_::Data> - Data objects

L<SOAP_::Header> - Header Data Objects

L<SOAP_::Serializer> - Serializes data structures to SOAP messages

L<SOAP_::Deserializer> - Deserializes SOAP messages into SOAP_::SOM objects

L<SOAP_::SOM> - SOAP Message objects

L<SOAP_::Constants> - Provides access to common constants and defaults

L<SOAP_::Trace> - Tracing facilities

L<SOAP_::Schema> - Provides access and stub(s) for schema(s)

L<SOAP_::Schema::WSDL|SOAP_::Schema/SOAP_::Schema::WSDL> - WSDL implementation for SOAP_::Schema

L<SOAP_::Server> - Handles requests on server side

SOAP_::Server::Object - Handles objects-by-reference

L<SOAP_::Fault> - Provides support for Faults on server side

L<SOAP_::Utils> - A set of private and public utility subroutines

=item F<lib/SOAP/Packager.pm>

L<SOAP_::Packager> - Provides an abstract class for implementing custom packagers.

L<SOAP_::Packager::MIME|SOAP_::Packager/SOAP_::Packager::MIME> - Provides MIME support to SOAP_::Lite

L<SOAP_::Packager::DIME|SOAP_::Packager/SOAP_::Packager::DIME> - Provides DIME support to SOAP_::Lite

=item F<lib/SOAP/Transport/HTTP.pm>

L<SOAP_::Transport::HTTP::Client|SOAP_::Transport/SOAP_::Transport::HTTP::Client> - Client interface to HTTP transport

L<SOAP_::Transport::HTTP::Server|SOAP_::Transport/SOAP_::Transport::HTTP::Server> - Server interface to HTTP transport

L<SOAP_::Transport::HTTP::CGI|SOAP_::Transport/SOAP_::Transport::HTTP::CGI> - CGI implementation of server interface

L<SOAP_::Transport::HTTP::Daemon|SOAP_::Transport/SOAP_::Transport::HTTP::Daemon> - Daemon implementation of server interface

L<SOAP_::Transport::HTTP::Apache|SOAP_::Transport/SOAP_::Transport::HTTP::Apache> - mod_perl implementation of server interface

=item F<lib/SOAP/Transport/POP3.pm>

L<SOAP_::Transport::POP3::Server|SOAP_::Transport/SOAP_::Transport::POP3::Server> - Server interface to POP3 protocol

=item F<lib/SOAP/Transport/MAILTO.pm>

L<SOAP_::Transport::MAILTO::Client|SOAP_::Transport/SOAP_::Transport::MAILTO::Client> - Client interface to SMTP/sendmail

=item F<lib/SOAP/Transport/LOCAL.pm>

L<SOAP_::Transport::LOCAL::Client|SOAP_::Transport/SOAP_::Transport::LOCAL::Client> - Client interface to local transport

=item F<lib/SOAP/Transport/TCP.pm>

L<SOAP_::Transport::TCP::Server|SOAP_::Transport/SOAP_::Transport::TCP::Server> - Server interface to TCP protocol

L<SOAP_::Transport::TCP::Client|SOAP_::Transport/SOAP_::Transport::TCP::Client> - Client interface to TCP protocol

=item F<lib/SOAP/Transport/IO.pm>

L<SOAP_::Transport::IO::Server|SOAP_::Transport/SOAP_::Transport::IO::Server> - Server interface to IO transport
...
=item new(optional key/value pairs)
    ...
    $client = SOAP_::Lite->new(proxy => $endpoint)
...
See L<SOAP_::Transport> for details.
...
See L<SOAP_::Serializer> for details.

=item packager(optional packager object)

    $packager = $client->packager( )

Provides access to the C<SOAP_::Packager> object that the client uses to manage
...
This method is actually an alias to the proxy method of L<SOAP_::Transport>.
...
See L<COMPRESSION|SOAP_::Transport/"COMPRESSION"> in L<HTTP::Transport>.
...
For example, if you wish to set the HTTP timeout for a SOAP_::Lite client to 5
...
C<SOAP_::Lite> offers some support for creating method stubs from service
...
The default is to return the a L<SOAP_::SOM> object (false).
...
object. See the section on L<SOAP_::Serializer>.
...
accessed by C<SOAP_::SOM::parts()>.

=item self

    $ref = SOAP_::Lite->self;

Returns an object reference to the default global object the C<SOAP_::Lite>
...
As has been illustrated in previous chapters, the C<SOAP_::Lite> client objects
...
from a L<SOAP_::Data> object, so as to allow full control over the namespace
...
The next four methods used in the C<SOAP_::Lite> class are geared towards
...
To change SOAP_::Lite's behaviour to use uri/method as SOAPAction header, use
the following code:
...
(which is covered shortly under the L<SOAP_::SOM> methods).
...
A client producing such a call is implemented like this:
 ...
 use SOAP_::Lite;
 my $soap = SOAP_::Lite->new( proxy => 'http://localhost:81/soap-wsdl-test/helloworld.pl');
...
A web service client using this schema looks like this:
 ...
 use SOAP_::Lite;
 my $soap = SOAP_::Lite->service("file:say_hello_rpcenc.wsdl");
...
You may of course also use the following one-liner:
 ...
 perl -MSOAP_::Lite -e 'print SOAP_::Lite->service("file:say_hello_rpcenc.wsdl")\
...
A web service client (without a service description) looks like this.
 ...
 my $som = $soap->call('sayHello',
    SOAP_::Data->name('name')->value('Kutter'),
    SOAP_::Data->name('givenName')->value('Martin')
 );
 die $som->faultstring if ($som->fault);
 print $som->result, "\n";
...
This is the SOAP_::Lite implementation for the web service client:

 use SOAP_::Lite +trace;
 my $soap = SOAP_::Lite->new( proxy => 'http://localhost:80/helloworld.pl');

 $soap->on_action( sub { "urn:HelloWorld#sayHello" });
 $soap->autotype(0)->readable(1);
 $soap->default_ns('urn:HelloWorld');

 my $som = $soap->call('sayHello', SOAP_::Data->name('parameters')->value(
    \SOAP_::Data->value([
        SOAP_::Data->name('name')->value( 'Kutter' ),
        SOAP_::Data->name('givenName')->value('Martin'),
    ]))
...
SOAP_::Lite's point of view, the only differences between rpc/literal and
...
While SOAP_::Data provides full control over the XML generated, passing
hash-like structures require additional coding.

=head1 WRITING A SOAP SERVER

See L<SOAP_::Server>, or L<SOAP_::Transport>.

=head1 FEATURES

=head2 ATTACHMENTS

C<SOAP_::Lite> features support for the SOAP with Attachments specification.
Currently, SOAP_::Lite only supports MIME based attachments. DIME based
...
C<SOAP_::Lite> clients can specify attachments to be sent along with a request
by using the C<SOAP_::Lite::parts()> method, which takes as an argument an
...
Servers, like clients, use the S<SOAP_::SOM> module to access attachments
transmitted to it.
  ...
  use strict;
  use vars qw(@ISA);
  @ISA = qw(SOAP_::Server::Parameters);
  sub someMethod {
    my $self = shift;
    my $envelope = pop;
...
C<MIME::Entity> objects along with SOAP_::Data elements, or any other data
intended for the response.
  ...
  sub someMethod {
    ...
    'Filename'    => "some.xml",
    'Disposition' => "attachment";
    return SOAP_::Data->name("foo" => "blah blah blah"),$ent;
  }
...
C<SOAP_::Lite> objects in your application you may do:

  use SOAP_::Lite
    proxy => 'http://localhost/cgi-bin/soap.cgi',
    uri => 'http://my.own.com/My/Examples';

  my $soap1 = new SOAP_::Lite; # will get the same proxy()/uri() as above
  print $soap1->getStateName(1)->result;

  my $soap2 = SOAP_::Lite->new; # same thing as above
  print $soap2->getStateName(2)->result;

  # or you may override any settings you want
  my $soap3 = SOAP_::Lite->proxy('http://localhost/');
  print $soap3->getStateName(1)->result;

B<Any> C<SOAP_::Lite> properties can be propagated this way. Changes in object
copies will not affect global settings and you may still change global
settings with C<< SOAP_::Lite->self >> call which returns reference to global
object. Provided parameter will update this object and you can even set it to
C<undef>:

  SOAP_::Lite->self(undef);

The C<use SOAP_::Lite> syntax also lets you specify default event handlers for
...
unexpected results. Consider code:
  ...
  use SOAP_::Lite proxy => 'http://localhost/';
  print SOAP_::Lite->getStateName(1)->result;

  use SOAP_::Lite proxy => 'http://localhost/cgi-bin/soap.cgi';
  print SOAP_::Lite->getStateName(1)->result;

B<Both> SOAP calls will go to C<'http://localhost/cgi-bin/soap.cgi'>. If you
want to execute C<use> at run-time, put it in C<eval>:

  eval "use SOAP_::Lite proxy => 'http://localhost/cgi-bin/soap.cgi'; 1" or die;

Or alternatively,

  SOAP_::Lite->self->proxy('http://localhost/cgi-bin/soap.cgi');

=head2 SETTING MAXIMUM MESSAGE SIZE

One feature of C<SOAP_::Lite> is the ability to control the maximum size of a
message a SOAP_::Lite server will be allowed to process. To control this
feature simply define C<$SOAP_::Constants::MAX_CONTENT_SIZE> in your code like
so:

  use SOAP_::Transport::HTTP;
  use MIME::Entity;
  $SOAP_::Constants::MAX_CONTENT_SIZE = 10000;
  SOAP_::Transport::HTTP::CGI
    ->dispatch_to('TemperatureService')
    ->handle;

=head2 IN/OUT, OUT PARAMETERS AND AUTOBINDING

C<SOAP_::Lite> gives you access to all parameters (both in/out and out) and
...
B<Server Code>:
  ...
  sub mymethod {
    ...
    my $param1 = shift;
    my $param2 = SOAP_::Data->name('myparam' => shift() * 2);
...
C<method> or C<PATH/> when creating C<use>ing the SOAP_::Lite module. The
...
In addition to this C<SOAP_::Lite> also supports an experimental syntax that
...
See also:
L<EXAMPLE APACHE::REGISTRY USAGE|SOAP_::Transport/"EXAMPLE APACHE::REGISTRY USAGE">,
L</"SECURITY">

=head2 COMPRESSION

C<SOAP_::Lite> provides you option to enable transparent compression over the
...
B<Server Code>
  ...
  my $server = SOAP_::Transport::HTTP::CGI
    ->dispatch_to('My::Parameters')
    ->options({compress_threshold => 10000})
    ->handle;

For more information see L<COMPRESSION|SOAP_::Transport/"COMPRESSION"> in
L<HTTP::Transport>.
...
=head2 Microsoft .NET client with SOAP_::Lite Server

In order to use a .NET client with a SOAP_::Lite server, be sure you use fully
qualified names for your return values. For example:

  return SOAP_::Data->name('myname')
                   ->type('string')
                   ->uri($MY_NAMESPACE)
                   ->value($output);
...
=head2 SOAP_::Lite client with a .NET server

If experiencing problems when using a SOAP_::Lite client to call a .NET Web
...
the Content-type header to be a mimetype exclusively, but SOAP_::Lite specifies
...
To turn off this behavior specify use the following code:
  ...
  use SOAP_::Lite;
  $SOAP_::Constants::DO_NOT_USE_CHARSET = 1;
...
SOAP_::Lite allows users to disable the use of explicit namespaces through the
C<use_prefix()> method. For example, the following code:

  $som = SOAP_::Lite->uri('urn:MyURI')
                   ->proxy($HOST)
                   ->use_prefix(0)
                   ->myMethod();
...
SOAP_::Lite uses the SOAP encoding (section 5 of the soap 1.1 spec), and
...
=item SOAP_::Lite serializes "18373" as an integer, but I want it to be a string!

SOAP_::Lite guesses datatypes from the content provided, using a set of
common-sense rules. These rules are not 100% reliable, though they fit for
most data.

You may force the type by passing a SOAP_::Data object with a type specified:

 my $proxy = SOAP_::Lite->proxy('http://www.example.org/soapservice');
 my $som = $proxy->myMethod(
     SOAP_::Data->name('foo')->value(12345)->type('string')
 );

You may also change the precedence of the type-guessing rules. Note that this
means fiddling with SOAP_::Lite's internals - this may not work as
...
the string test is normally last and allways returns true):
  ...
  my $proxy = SOAP_::Lite->uri($uri)->proxy($proxyUrl);
  $proxy->serializer->typelookup->{string}->[0] = 0;
  $proxy->myMethod(\@list);

See L<SOAP_::Serializer|SOAP_::Serializer/AUTOTYPING> for more details.
...
You would do something like this:
   ...
   use SOAP_::Lite dispatch_from(Some::Module) =>
       uri => 'urn:Foo'
       proxy => 'http://...';

=item Problems using SOAP_::Lite's COM Interface
...
Using C<SOAP_::Lite> (or L<XML::Parser::Expat>) in combination with mod_perl
...
=item Java SAX parser unable to parse message composed by SOAP_::Lite

In some cases SOAP messages created by C<SOAP_::Lite> may not be parsed
...
C<SOAP_::Lite> is based on L<XML::Parser> which is basically wrapper around
...
sender's side. It will definitely work with SOAP_::Lite and it B<may> work with
...
C<< SOAP_::Data->type(base64 => $string) >> either on client or on server
side. If you want change behavior for specific instance of SOAP_::Lite, you
may subclass C<SOAP_::Serializer>, override C<as_string()> method that is
responsible for string encoding (take a look into C<as_base64Binary()>) and
specify B<new> serializer class for your SOAP_::Lite object with:

  my $soap = new SOAP_::Lite
    serializer => My::Serializer->new,
    ..... other parameters

or on server side:

  my $server = new SOAP_::Transport::HTTP::Daemon # or any other server
    serializer => My::Serializer->new,
    ..... other parameters

If you want to change this behavior for B<all> instances of SOAP_::Lite, just
substitute C<as_string()> method with C<as_base64Binary()> somewhere in your
code B<after> C<use SOAP_::Lite> and B<before> actual processing/sending:

  *SOAP_::Serializer::as_string = \&SOAP_::XMLSchema2001::Serializer::as_base64Binary;

Be warned that last two methods will affect B<all> strings and convert them
into base64 encoded. It doesn't make any difference for SOAP_::Lite, but it
...
and you can add your own implementation with SOAP_::Data).
...
SOAP_::Lite allows to add support for additional transport protocols, or
server handlers, via separate modules implementing the SOAP_::Transport::*
...
L<SOAP_::Transport::HTTP::Nginx|SOAP_::Transport::HTTP::Nginx> provides a transport module for nginx (<http://nginx.net/>)

=back

=head1 AVAILABILITY

You can download the latest version SOAP_::Lite for Unix or SOAP_::Lite for
...
You are welcome to send e-mail to the maintainers of SOAP_::Lite with your
...
documentation effort of SOAP_::Lite.

Special thanks to O'Reilly publishing which has graciously allowed SOAP_::Lite
to republish and redistribute the SOAP_::Lite reference manual found in
...
SOAP_::Lite's development takes place on sourceforge.net.
...
Please report all suspected SOAP_::Lite bugs using Sourceforge. This ensures
