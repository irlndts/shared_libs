...
use SOAP_::WSDL::Factory::Deserializer;
use SOAP_::WSDL::Factory::Serializer;
use SOAP_::WSDL::Factory::Transport;
use SOAP_::WSDL::Expat::MessageParser;
...
sub set_proxy {
    ...
    # set transport - SOAP_::Lite works similar...
    $transport_of{ $ident } =  SOAP_::WSDL::Factory::Transport
...
# Mimic SOAP_::Lite's behaviour for getter/setter routines
SUBFACTORY: {
...
sub call {
    ...
    my ($operation, $soap_action) = (ref $method eq 'HASH')
        ...
        : (blessed $data
            && $data->isa('SOAP_::WSDL::XSD::Typelib::Builtin::anyType'))
            ? ( $method , (join q{/}, $data->get_xmlns(), $method) )
            : ( $method, q{} );
    $serializer_of{ $ident } ||= SOAP_::WSDL::Factory::Serializer->get_serializer({
    ...
    # Normally, SOAP_::Lite's transport layer is used, though users
    ...
    # get deserializer
    use Data::Dumper;
    $deserializer_of{ $ident } ||= SOAP_::WSDL::Factory::Deserializer->get_deserializer({
    ...
    if ( $response ) {
        ...
        elsif (blessed $@) { #}&& $@->isa('SOAP_::WSDL::SOAP_::Typelib::Fault11')) {
            return $@;
        }
        else {
