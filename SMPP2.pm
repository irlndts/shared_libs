...
sub new_transceiver {
    ...
    #if (!defined ${*$me}{Blocking}) { ${*$me}{Blocking} = 1 };
...
sub explain_status {
    ...
    return sprintf("%s (%s=0x%08X)",
		   SMPP2::status_code->{$me->{status}}->{msg},
		   SMPP2::status_code->{$me->{status}}->{code},
		   $me->{status});
}
...
sub read_pdu {
    ...
    my $bytes_read = $me->read_hard($head_len, \$header, 0);
    if ($bytes_read == 0) { return undef };
